...
* ```algo_params``` folder contains the JSON schema file to validate algorithm configuration files as well as examples of such files.

* ```configuration``` folder contains the JSON schema file to validate pipeline configuration files as well as examples of such files.

* ```database``` folder contains the dump of each _MongoDB_ database (total of 3) deployed on the _LIARA-SHCA_ architecture.

* ```input_data``` folder contains examples of raw data from various sources.
